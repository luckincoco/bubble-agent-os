...
function emptyLine(key: number): LineState {
  return {
    key, brand: '', material: '', spec: '', unit: 'ton',
    weighMode: '理计', bundleCount: '', weightPerPc: '', quantity: '',
    unitPrice: '', taxInclusive: true, notes: '',
  }
}

function calcLineQty(line: LineState): number {
  if (line.weighMode === '理计') {
    const bc = parseInt(line.bundleCount) || 0
    const wpp = parseFloat(line.weightPerPc) || 0
    return bc * wpp
  }
...
export function PurchaseEventForm({ onSuccess }: { onSuccess?: () => void } = {}) {
  ...
  /** Find or create a product, return its ID */
  ...
  const handleSubmit = async () => {
    if (!supplierId) { setMsg('请选择供应商'); return }
