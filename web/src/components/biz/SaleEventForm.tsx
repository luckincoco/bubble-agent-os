...
import { createSaleWithLinesApi, type BizLineInput } from '../../services/api'
...
export function SaleEventForm({ onSuccess }: { onSuccess?: () => void } = {}) {
  const { products, counterparties, projects, addProduct, addCounterparty, addProject, loadSales } = useBizStore()
  ...
  const updateLine = (idx: number, updated: LineState) => setLines(prev => prev.map((l, i) => i === idx ? updated : l))
  const removeLine = (idx: number) => { if (lines.length > 1) setLines(prev => prev.filter((_, i) => i !== idx)) }
  const addLine = () => { setLines(prev => [...prev, emptyLine(lineKey)]); setLineKey(k => k + 1) }
    ...
    try {
      const lineInputs: BizLineInput[] = await Promise.all(
        validLines.map(async (l) => {
          ...
          return {
            productId, brand: l.brand, material: l.material, spec: l.spec,
            measureUnit: UNIT_LABELS[l.unit], weighMode: l.weighMode,
            ...
            quantity: qty, unitPrice: parseFloat(l.unitPrice) || 0,
      ...
      await createSaleWithLinesApi({
        date, customerId,
        location: location || undefined, docNo: docNo || undefined,
        projectId: projectId || undefined, notes: notes || undefined,
      ...
      setMsg('销售事件录入成功!')
      setLines([emptyLine(lineKey)]); setLineKey(k => k + 1)
      setDocNo(''); setPaidAmount(''); setPaymentMethod(''); setNotes('')
      loadSales(); onSuccess?.()
  ...
  return (
    <div className={s.form}>
      ...
      {lines.map((line, idx) => (
        <LineItemCard key={line.key} line={line} index={idx} products={products}
          onChange={(updated) => updateLine(idx, updated)} onRemove={() => removeLine(idx)} />
      ))}
      ...
      <div className={s.section}><span>收款信息（可选）</span></div>
      <div className={s.row}>
